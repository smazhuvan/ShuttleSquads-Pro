...
import { Trophy, Activity, Flame, RefreshCw, Zap, Swords, BarChart3, Cpu, ChevronRight } from 'lucide-react';
...
export default function ShuttleSquadsPro() {
  ...
  const [activeTab, setActiveTab] = useState('insights'); // Defaulting to insights so you can see the revamp immediately!
  ...
  // Helper to format "Dinesh - Pramoth" into "DIN/PRA" for tight spaces
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-4 md:p-8">
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="bg-white/70 backdrop-blur-xl border border-white p-6 rounded-3xl shadow-xl shadow-slate-200/50">
            ...
            <div className="flex gap-2">
              <button onClick={fetchData} disabled={loading} className="flex-1 bg-gradient-to-br from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-indigo-200 active:scale-95 flex justify-center items-center gap-2">
        ...
        <div className="xl:col-span-4 space-y-8">
          ...
          {data && data.length > 0 && (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
              ...
              {/* TABS NAVIGATION */}
              <div className="flex bg-white/50 backdrop-blur-md p-2 rounded-2xl w-full overflow-x-auto no-scrollbar border border-slate-200 shadow-sm">
              ...
              <div className="bg-white/90 backdrop-blur-2xl border border-white rounded-[2.5rem] p-6 md:p-10 shadow-2xl shadow-slate-200/50">
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-4">
                                ...
                                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
                                  <div className="h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${(team.power_rating / 1700) * 100}%`, backgroundColor: team.color }}></div>
                                </div>
                              </div>
                            </td>
                ...
                {activeTab === 'h2h' && (
                  <div className="space-y-8 max-w-4xl mx-auto">
                    ...
                    })() : <div className="text-center p-8 bg-amber-50 border border-amber-100 text-amber-600 rounded-3xl font-bold flex flex-col items-center gap-3"><Activity size={32}/> Please select two different franchises to simulate a matchup.</div>}
                  </div>
                )}

                {/* 3. VISUAL INSIGHTS - REVAMPED FOR MASSIVE SCALE */}
                {activeTab === 'insights' && (
                  <div className="space-y-16">
                    
                    {/* SCATTER CHART - DYNAMIC HEIGHT */}
                    <div>
                      <div className="mb-8">
                        ...
                        <p className="text-slate-500 text-sm mt-1">Glicko-2 ratings mapped across the entire tournament field.</p>
                      </div>
                      
                      {/* Dynamic Height based on number of teams so it NEVER squishes */}
                      <div className="w-full" style={{ height: `${Math.max(400, data.length * 45)}px` }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <ScatterChart margin={{ top: 20, right: 40, bottom: 20, left: 120 }}>
                            ...
                            {/* X Axis is now the Power Rating */}
                            <XAxis 
                              type="number" 
                              dataKey="power_rating" 
                              name="Power" 
                            ...
                            {/* Y Axis is the Teams, given plenty of width (120px) */}
                            <YAxis 
                              type="category" 
                              dataKey="team" 
                              name="Team" 
                            ...
                            <Scatter data={data} shape="circle">
                              {data.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.color} className="drop-shadow-md hover:drop-shadow-xl transition-all" />
                              ))}
                            </Scatter>
                          </ScatterChart>
                        </ResponsiveContainer>
                    ...
                    {/* HEATMAP - STICKY HEADERS & LARGER CELLS */}
                    <div>
                      <div className="mb-8">
                        ...
                        <p className="text-slate-500 text-sm mt-1">Row team's mathematical probability of defeating the Column team.</p>
                      </div>

                      <div className="relative rounded-2xl border border-slate-200 shadow-sm overflow-hidden bg-white">
                        <div className="overflow-x-auto pb-2 custom-scrollbar">
                          {/* Matrix Container - Dynamically sized based on columns */}
                          <div className="min-w-max grid" style={{ gridTemplateColumns: `160px repeat(${data.length}, 56px)` }}>
                            ...
                            {data.map(t => (
                              <div key={`header-${t.team}`} className="h-16 flex items-end justify-center pb-3 border-b border-slate-200 bg-slate-50 relative group">
                                <span className="text-[10px] font-black text-slate-500 -rotate-45 whitespace-nowrap origin-bottom-left cursor-help">
                                  {formatShortName(t.team)}
                                </span>
                                {/* Tooltip for full name on hover */}
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-slate-800 text-white text-[10px] py-1 px-2 rounded whitespace-nowrap z-50">
                                  {t.team}
                                </div>
                              </div>
                            ))}
                            ...
                            {matrix.map((row, rIdx) => (
                              <React.Fragment key={`row-${row.name}`}>
                                ...
                                <div className={`h-12 sticky left-0 z-20 flex items-center justify-end pr-4 text-[11px] font-black text-slate-700 border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] ${rIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                  <span className="truncate max-w-[130px]" title={row.name}>{row.name}</span>
                                </div>
                                
                                {/* Data Cells */}
                                {data.map(col => {
                                  const prob = row[col.team];
                                  const isSelf = row.name === col.team;
                                  
                                  if (isSelf) {
                                    return <div key={`${row.name}-${col.team}`} className="h-12 border-b border-r border-slate-100 bg-slate-100/50 flex items-center justify-center text-slate-300">➖</div>;
                                  }

                                  return (
                                    <div key={`${row.name}-${col.team}`} 
                                         className={`h-12 border-b border-r border-white/20 flex items-center justify-center text-[11px] cursor-help transition-all hover:scale-110 hover:z-10 hover:shadow-lg rounded-[4px] m-[1px] ${getHeatmapColor(prob)}`}
                                         title={`${row.name} has a ${(prob*100).toFixed(1)}% chance to beat ${col.team}`}>
                                      {(prob*100).toFixed(0)}
                                    </div>
                                  )
                                })}
                      ...
                      <div className="flex items-center justify-end gap-4 mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-red-500"></div> Underdog</div>
                        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-yellow-400"></div> Toss-up</div>
                        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-emerald-500"></div> Favorite</div>
                      </div>
      ...
      {/* Add a tiny bit of CSS for the custom scrollbar */}
...
function TabButton({ active, onClick, icon, label }) {
  return (
    <button 
      onClick={onClick}
      className={`flex-1 py-3.5 px-4 rounded-[14px] text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 whitespace-nowrap
