...
import { Trophy, Activity, Flame, RefreshCw, Zap, Swords, BarChart3, Cpu, ChevronRight, Target, Binary, GitMerge } from 'lucide-react';
...
export default function ShuttleSquadsPro() {
  ...
  const [bracket, setBracket] = useState(null); // New State for Bracket
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      } catch (e) { console.warn("Futures pending..."); }
      ...
      try {
        ...
        if (!bracketJson.error) {
            setBracket(bracketJson.rounds);
        } else {
            // Mocking for UI development if API is still being deployed
            setBracket([
                { name: "Quarter-Finals", matches: [
                    { t1: "Dinesh - Pramoth", t2: "Baskar - Ram", s1: 21, s2: 12, winner: "Dinesh - Pramoth" },
                    { t1: "Hariharan - Vaira", t2: "Shyam - Shajith", s1: null, s2: null, winner: null },
                    { t1: "Manikandan - Vignesh", t2: "Sandeep - Siva", s1: null, s2: null, winner: null },
                    { t1: "Satheesh - Vasanth", t2: "Ramesh - Vijay", s1: null, s2: null, winner: null }
                ]},
                { name: "Semi-Finals", matches: [
                    { t1: "Dinesh - Pramoth", t2: "TBD", s1: null, s2: null, winner: null },
                    { t1: "TBD", t2: "TBD", s1: null, s2: null, winner: null }
                ]},
      ...
      } catch (e) { console.warn("Bracket engine syncing..."); }
  ...
  const predictScoreline = (probA) => {
    if (probA > 0.5) return { sa: 21, sb: Math.min(Math.max(Math.floor(21 * (1 - probA) * 1.8), 0), 19) };
  ...
  const getHeatmapColor = (prob) => {
    if (prob > 0.8) return 'bg-emerald-500 text-white font-black';
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white/90 backdrop-blur-xl p-4 border border-slate-100 rounded-2xl shadow-xl">
          <p className="font-black text-slate-800 text-sm">{d.team}</p>
          <p className="text-lg font-black text-indigo-600">{d.power_rating} ⚡</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        {data && <div className="bg-indigo-600 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">Telemetry Active</div>}
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-5 gap-8">
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="bg-white/70 backdrop-blur-xl border border-white p-6 rounded-3xl shadow-xl shadow-slate-200/50">
            ...
            <input type="text" value={tournamentId} onChange={(e) => setTournamentId(e.target.value)} className="w-full bg-slate-100/50 border border-slate-200 p-3 rounded-xl mb-4 font-mono text-xs text-slate-600 outline-none" />
            <button onClick={fetchData} disabled={loading} className="w-full bg-gradient-to-br from-indigo-600 to-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg active:scale-95 flex justify-center items-center gap-2">
          ...
          <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl space-y-4">
            <h3 className="font-black text-sm uppercase tracking-widest text-indigo-400 flex items-center gap-2">
                <Binary size={16}/> Logic Stack
            </h3>
            <p className="text-[11px] text-slate-400 leading-relaxed">Glicko-2 ratings drive the probability engine. Monte Carlo runs 10k simulations per fetch.</p>
          </div>
        </div>
        ...
        <div className="xl:col-span-4 space-y-8">
          ...
          {error && <div className="bg-red-50 text-red-600 p-4 rounded-2xl border border-red-200 font-bold">{error}</div>}
          
          {(!data || data.length === 0) && !loading && !error && (
            <div className="bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] p-16 text-center flex flex-col items-center justify-center">
          ...
          {data && data.length > 0 && (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
              ...
              <div className="bg-white/90 backdrop-blur-2xl border border-white rounded-[2.5rem] p-6 md:p-10 shadow-2xl shadow-slate-200/50">
                ...
                {/* 1. TOURNAMENT BRACKET (New Feature) */}
                {activeTab === 'bracket' && (
                ...
                {activeTab === 'futures' && (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between mb-8">
                      <div><h4 className="font-black text-2xl text-slate-800">Tournament Forecast</h4><p className="text-slate-500 text-sm">10,000 Simulation Runs</p></div>
                    </div>
                    {!futures ? <p className="text-center p-12 text-slate-400 font-bold">Waiting for 8 qualified teams...</p> : (
                        <div className="grid grid-cols-1 gap-4">
                        {futures.map((f, i) => (
                            <div key={f.team} className="bg-slate-50/50 border border-slate-100 rounded-2xl p-6 hover:bg-white transition-all">
                                <div className="flex justify-between items-center mb-4">
                                    <h5 className="font-black text-slate-800">{f.team} <span className="text-indigo-400 text-[10px] ml-2">{f.power_rating} PWR</span></h5>
                                    <div className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-black text-sm">{f.win_championship}% <span className="text-[8px] opacity-70 ml-1 uppercase">Win</span></div>
                                </div>
                                <div className="grid grid-cols-3 gap-1 h-2 rounded-full overflow-hidden bg-slate-200/40">
                ...
                {activeTab === 'h2h' && (
                  <div className="space-y-8 max-w-4xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-7 gap-4 items-center">
                      <div className="md:col-span-3 bg-white border border-slate-200 p-8 rounded-3xl shadow-sm relative overflow-hidden">
                        <label className="text-[10px] font-black uppercase text-blue-500 mb-3 block">Blue Corner</label>
                        <select className="w-full bg-slate-50 p-4 rounded-xl font-bold outline-none" value={teamA} onChange={(e) => setTeamA(e.target.value)}>{data.map(t => <option key={t.team} value={t.team}>{t.team}</option>)}</select>
                      </div>
                      <div className="md:col-span-1 flex justify-center"><Swords size={24} className="text-slate-300" /></div>
                      <div className="md:col-span-3 bg-white border border-slate-200 p-8 rounded-3xl shadow-sm relative overflow-hidden">
                        <label className="text-[10px] font-black uppercase text-red-500 mb-3 block">Red Corner</label>
                        <select className="w-full bg-slate-50 p-4 rounded-xl font-bold outline-none" value={teamB} onChange={(e) => setTeamB(e.target.value)}>{data.map(t => <option key={t.team} value={t.team}>{t.team}</option>)}</select>
                      </div>
                    </div>
                    {teamA !== teamB ? (() => {
                          ...
                          <div className="min-w-max grid" style={{ gridTemplateColumns: `160px repeat(${data.length}, 56px)` }}>
                            ...
                            {matrix.map((row, rIdx) => (
                              <React.Fragment key={row.name}>
                                <div className={`h-12 sticky left-0 z-20 flex items-center justify-end pr-4 text-[11px] font-black text-slate-700 border-r border-slate-200 ${rIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                ...
                                {data.map(col => {
                                  ...
                                  return (
                                    <div key={`${row.name}-${col.team}`} className={`h-12 border-b border-r border-white/20 flex items-center justify-center text-[11px] m-[1px] ${getHeatmapColor(prob)}`}>
                                      {(prob*100).toFixed(0)}
                                    </div>
                                  )
                                })}
...
function MetricCard({ icon, title, value, sub }) {
  return (
    <div className="bg-white/80 backdrop-blur-xl border border-white p-6 rounded-3xl shadow-lg flex flex-col items-center text-center">
      <div className="p-4 bg-slate-50 rounded-full mb-4">{icon}</div>
