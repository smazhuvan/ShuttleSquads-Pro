...
export default function ShuttleSquadsPro() {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white/90 dark:bg-slate-900/90 dark:border-slate-700 backdrop-blur-xl p-4 border border-slate-100 rounded-2xl shadow-xl">
          <p className="font-black text-slate-800 dark:text-slate-100 text-sm mb-1">{d.team}</p>
  ...
  return (
    <div className={isDark ? "dark" : ""}>
      <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-50 via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 font-sans p-4 md:p-8 overflow-hidden relative transition-colors duration-500">
        
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-blue-400/20 dark:bg-blue-600/10 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen pointer-events-none animate-blob"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-indigo-400/20 dark:bg-indigo-600/10 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen pointer-events-none animate-blob animation-delay-2000"></div>
          ...
          <div className="flex items-center gap-3">
              {data && (
                <div className="relative group">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-2xl blur opacity-30 group-hover:opacity-60 transition duration-1000 animate-pulse"></div>
                  <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-indigo-600 dark:text-indigo-400 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></div> Sync Active
                  </div>
                </div>
              )}
              ...
              <button onClick={() => setIsShareOpen(true)} className="group bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 p-3 rounded-2xl hover:bg-indigo-50 dark:hover:bg-slate-700 transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-1">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-5 gap-8 relative z-10">
          ...
          <div className="xl:col-span-1 space-y-6">
            <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl border border-white dark:border-slate-700/50 p-6 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 group animate-fade-in-left">
              <div className="flex items-center gap-3 mb-5 text-indigo-600 dark:text-indigo-400 font-black">
                <div className="p-2 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl group-hover:bg-indigo-100 dark:group-hover:bg-indigo-500/20 transition-colors"><Zap size={20} className="fill-indigo-500 dark:fill-indigo-400" /></div>
                <h2 className="text-lg tracking-tight">Oracle Link</h2>
              </div>
              <input type="text" value={tournamentId} onChange={(e) => setTournamentId(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-700/50 p-4 rounded-xl mb-4 font-mono text-xs text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all duration-300" placeholder="e.g., c2008" />
              <button onClick={fetchData} disabled={loading} className="relative w-full overflow-hidden rounded-xl group/btn active:scale-95 transition-transform">
                <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-indigo-600 to-blue-600 dark:from-indigo-500 dark:to-blue-600 group-hover/btn:scale-105 transition-transform duration-500"></div>
          ...
          <div className="xl:col-span-4 space-y-8 min-h-[600px]">
            {error && <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-6 rounded-2xl border border-red-200 dark:border-red-900/50 font-bold shadow-sm animate-shake">{error}</div>}
            
            {(!data || data.length === 0) && !loading && !error && (
              <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border-2 border-dashed border-slate-300/50 dark:border-slate-700/50 rounded-[3rem] p-20 text-center flex flex-col items-center justify-center animate-fade-in-up h-full">
                <div className="relative mb-6">
                  <div className="absolute inset-0 bg-indigo-500 blur-xl opacity-20 rounded-full animate-pulse"></div>
                  <Trophy size={80} className="text-slate-300 dark:text-slate-700 relative z-10" />
                </div>
                <h3 className="text-slate-600 dark:text-slate-400 font-black text-3xl tracking-tight">Awaiting Telemetry Sync</h3>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-4 max-w-sm leading-relaxed">Connect to a tournament short-code or UUID to initialize the Monte Carlo engine.</p>
              </div>
            )}
            ...
            {data && data.length > 0 && (
              <div className="space-y-8">
                ...
                <div className="flex bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl p-2 rounded-[1.5rem] w-full overflow-x-auto no-scrollbar border border-white dark:border-slate-700/50 shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 gap-2 animate-fade-in-up animation-delay-200">
                ...
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-white dark:border-slate-700/50 rounded-[3rem] p-6 md:p-10 shadow-2xl shadow-slate-300/40 dark:shadow-slate-900/50 relative overflow-hidden animate-fade-in-up animation-delay-300">
                  ...
                  {activeTab === 'leaderboard' && (
                    <div className="overflow-x-auto custom-scrollbar pb-4 animate-fade-in">
                      <table className="w-full text-left border-collapse min-w-[900px]">
                        ...
                        <tbody>
                          {data.map((team, idx) => (
                            <tr key={team.team} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-white dark:hover:bg-slate-800/50 transition-all duration-300 group animate-slide-up" style={{ animationFillMode: 'both', animationDelay: `${idx * 40}ms` }}>
                          ...
                          <div className="flex justify-between min-w-[1000px] gap-12 px-4 py-4">
                              {!bracket ? <div className="flex justify-center w-full py-20"><RefreshCw className="animate-spin text-indigo-400" size={32}/></div> : bracket.map((round, rIdx) => (
                                  <div key={round.name} className="flex-1 space-y-8 animate-slide-up" style={{ animationFillMode: 'both', animationDelay: `${rIdx * 150}ms` }}>
                                      <h4 className="text-center font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-[0.2em] text-[10px] mb-10 bg-indigo-50 dark:bg-indigo-500/10 py-2 rounded-xl border border-indigo-100 dark:border-indigo-500/20">{round.name}</h4>
                                      <div className="flex flex-col justify-around h-full gap-10">
                                          {round.matches.map((m, mIdx) => (
                                              <div key={mIdx} className="relative group">
                                                  <div className="bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[1.5rem] overflow-hidden shadow-sm hover:shadow-xl hover:border-indigo-300 dark:hover:border-indigo-500 transition-all duration-300 w-full relative z-10 hover:-translate-y-1">
                                                      <div className={`p-4 md:p-5 flex justify-between items-center border-b border-slate-50 dark:border-slate-800 transition-colors ${m.winner === m.t1 ? 'bg-indigo-50/50 dark:bg-indigo-500/10' : ''}`}>
                                                          <span className={`text-sm font-bold truncate pr-4 ${m.winner === m.t1 ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300'}`}>{m.t1}</span>
                                                          <span className="font-mono font-black text-xs text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{m.s1 ?? '-'}</span>
                                                      </div>
                                                      <div className={`p-4 md:p-5 flex justify-between items-center transition-colors ${m.winner === m.t2 ? 'bg-indigo-50/50 dark:bg-indigo-500/10' : ''}`}>
                                                          <span className={`text-sm font-bold truncate pr-4 ${m.winner === m.t2 ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300'}`}>{m.t2}</span>
                                                          <span className="font-mono font-black text-xs text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{m.s2 ?? '-'}</span>
                                                      </div>
                                                  </div>
                                                  {rIdx < bracket.length - 1 && (
                                                      <div className="absolute -right-12 top-1/2 w-12 h-[2px] bg-slate-200 dark:bg-slate-700 group-hover:bg-indigo-400 transition-colors duration-300 origin-left"></div>
                                                  )}
                                              </div>
                                          ))}
                                      </div>
                  ...
                  {activeTab === 'futures' && (
                    <div className="space-y-8 animate-fade-in">
                      ...
                      {!futures ? <div className="flex justify-center py-20"><RefreshCw className="animate-spin text-slate-300" size={48}/></div> : (
                          ...
                          {futures.map((f, i) => (
                              <div key={f.team} className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[2rem] p-6 md:p-8 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 group animate-slide-up" style={{ animationFillMode: 'both', animationDelay: `${i * 100}ms` }}>
                                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                                      <div className="flex items-center gap-5">
                                          <div className="w-12 h-12 bg-slate-50 dark:bg-slate-900 rounded-2xl flex items-center justify-center font-black text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-700 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-colors duration-500 shadow-sm">{i+1}</div>
                                          <div>
                                              <h5 className="font-black text-xl text-slate-800 dark:text-slate-100 tracking-tight">{f.team}</h5>
                                              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1 bg-slate-100 dark:bg-slate-900 inline-block px-2 py-1 rounded-md">{f.power_rating} PWR</p>
                                          </div>
                                      </div>
                                      <div className="flex items-center gap-8">
                                          <div className="text-right hidden sm:block">
                                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Make Final</p>
                                              <p className="font-black text-slate-700 dark:text-slate-300 text-2xl">{f.make_finals}%</p>
                                          </div>
                                          <div className="text-center bg-gradient-to-br from-indigo-600 to-blue-600 text-white px-6 py-3 rounded-2xl shadow-lg shadow-indigo-200 dark:shadow-indigo-900/50 group-hover:scale-105 transition-transform duration-300">
                                  ...
                                  <div className="grid grid-cols-3 gap-2 h-3 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-900 shadow-inner border border-slate-200 dark:border-slate-700">
                                      <div className="h-full bg-blue-300 dark:bg-blue-500 group-hover:bg-blue-400 transition-colors duration-500 relative overflow-hidden" style={{ width: `${f.make_semis}%` }}><div className="absolute inset-0 bg-white/30 w-1/2 translate-x-full animate-shimmer"></div></div>
                                      <div className="h-full bg-indigo-500 dark:bg-indigo-400 group-hover:bg-indigo-600 transition-colors duration-500 relative overflow-hidden" style={{ width: `${f.make_finals}%` }}><div className="absolute inset-0 bg-white/20 w-1/2 translate-x-full animate-shimmer animation-delay-500"></div></div>
                                      <div className="h-full bg-amber-400 dark:bg-amber-500 group-hover:bg-amber-500 transition-colors duration-500 relative overflow-hidden" style={{ width: `${f.win_championship}%` }}><div className="absolute inset-0 bg-white/40 w-1/2 translate-x-full animate-shimmer animation-delay-1000"></div></div>
                                  </div>
                              </div>
                          ))}
                          </div>
                  ...
                  {activeTab === 'insights' && (
                    <div className="space-y-16 animate-fade-in">
                      <div className="h-[450px] bg-slate-50/50 dark:bg-slate-900/50 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800">
                        <h4 className="font-black text-lg mb-8 flex items-center gap-3"><Activity className="text-indigo-500 dark:text-indigo-400"/> Power Map Distribution</h4>
                        <ResponsiveContainer width="100%" height="100%">
                            <ScatterChart margin={{ top: 20, right: 40, bottom: 20, left: 120 }}>
                              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? '#334155' : '#e2e8f0'}/>
                              <XAxis type="number" dataKey="power_rating" domain={['dataMin - 50', 'dataMax + 50']} tick={{fill: isDark ? '#94a3b8' : '#64748b', fontSize: 12, fontWeight: 'bold'}} axisLine={false} tickLine={false} />
                              <YAxis type="category" dataKey="team" width={120} tick={{fill: isDark ? '#cbd5e1' : '#475569', fontSize: 11, fontWeight: 'bold'}} axisLine={false} tickLine={false} />
                              <RechartsTooltip content={<CustomTooltip />} cursor={{stroke: isDark ? '#475569' : '#cbd5e1', strokeWidth: 2, strokeDasharray: '5 5'}} />
        ...
        <div className="max-w-7xl mx-auto mt-16 pb-10 text-center relative z-10">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] flex items-center justify-center gap-2">
            ...
            <a href="https://github.com/smazhuvan" target="_blank" rel="noopener noreferrer" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 hover:underline underline-offset-4 transition-all">Shivakumar Mazhuvan</a>
          </p>
        </div>

        {/* SHARE MODAL */}
        {isShareOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-xl animate-fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 max-w-sm w-full shadow-2xl shadow-indigo-500/20 dark:shadow-indigo-900/50 space-y-8 text-center border border-white dark:border-slate-800 relative animate-scale-up">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-black text-2xl text-slate-800 dark:text-slate-100 tracking-tight">Share Oracle</h3>
                <button onClick={() => setIsShareOpen(false)} className="text-slate-400 hover:text-slate-800 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 w-8 h-8 rounded-full flex items-center justify-center transition-colors">✕</button>
              </div>
              <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] inline-block border border-slate-100 dark:border-slate-700 shadow-[inset_0_-4px_10px_rgba(0,0,0,0.02)]">
                <QRCodeSVG value={`${typeof window !== 'undefined' ? window.location.origin : ''}?tid=${tournamentId}`} size={220} fgColor={isDark ? "#818cf8" : "#4f46e5"} bgColor="transparent" includeMargin={false} />
              </div>
              <div className="space-y-4">
                <button onClick={handleNativeShare} className="w-full bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-500 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-3 shadow-xl hover:-translate-y-1 transition-all duration-300">
...
function MetricCard({ icon, title, value, sub, delay }) { 
  return (
    <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl border border-white dark:border-slate-800 p-6 md:p-8 rounded-[2rem] shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 flex flex-col items-center text-center hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-500/5 hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-all duration-500 group animate-slide-up" style={{ animationFillMode: 'both', animationDelay: `${delay}ms` }}>
      <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl mb-5 shadow-sm border border-slate-50 dark:border-slate-700 group-hover:scale-110 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-500/20 transition-all duration-500">{icon}</div>
      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-2" style={{minHeight: "15px"}}>{title}</p>
      <p className="text-xl md:text-2xl font-black text-slate-800 dark:text-slate-100 break-words leading-tight tracking-tight">{value}</p>
      <p className="text-xs font-bold text-indigo-500 dark:text-indigo-400 mt-2 bg-indigo-50 dark:bg-indigo-500/10 px-3 py-1 rounded-full">{sub}</p>
    </div>
  ); 
}
...
function TabButton({ active, onClick, icon, label }) { 
  return (
    <button onClick={onClick} className={`flex-1 py-4 px-5 rounded-[1.2rem] text-[10px] md:text-xs font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 whitespace-nowrap outline-none ${active ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-md border border-slate-100 dark:border-slate-700 scale-100' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-white/40 dark:hover:bg-slate-800/50 scale-95 hover:scale-100'}`}>
      {icon} {label}
    </button>
  ); 
}